...
return Application::configure(basePath: dirname(__DIR__))
    ...
    ->withSchedule(function (Schedule $schedule) {
        if (env('SCHEDULER_ENABLE')) {
            ...
            $schedule->job(new UpdateICNFData(3))->cron('0 2 */2 * *');
            $schedule->job(new UpdateICNFData(4))->cron('0 3 * * 1,5');
            $schedule->job(new UpdateICNFData(5))->cron('0 3 * * 1,5');
            $schedule->job(new UpdateICNFData(6))->cron('0 3 * * 3');
